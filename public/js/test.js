...
var view={
    ...
    width: 0.2,
    height: 0.2,
    background: new THREE.Color( 0.0, 0.5, 1 ),
    eye: [ 6, 5, 10 ],
    up: [ 0, 0, 1 ],
    fov: 60,
...
function init() {
    ...
    if (view){
        var vcamera = new THREE.PerspectiveCamera( view.fov, window.innerWidth / window.innerHeight, 0.5, 20 );
        vcamera.position.fromArray( view.eye );
        vcamera.up.fromArray( view.up );
        view.camera = vcamera;
        vcamera.lookAt( 1, 10, 10 );
        scene.add( new THREE.CameraHelper( vcamera ) );
